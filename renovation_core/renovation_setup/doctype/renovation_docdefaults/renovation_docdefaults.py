...
class RenovationDocDefaults(Document):
	...
	def fetch_fields(self, mandatory = 0):
		...
		# docfields
		docfields = frappe.get_all('DocField', fields=["name", "fieldname", "label", "fieldtype", "options"], filters=filters)
		
		# custom fields
		docfields.extend(frappe.get_all('Custom Field', fields=["name", "fieldname", "label", "fieldtype", "options"], filters=filters))
...
def fieldname_query(doctype, txt, searchfield, start, page_len, filters):
	...
	supported_types = ["'" + x + "'" for x in RenovationDocDefaults.supported_types]
