...
def batch_update(docs, commit_each_doc=True):
    ...
    action_map.save = ['create', 'save', 'update', 'edit'] + action_map.submit[1:]
            ...
            for data in list_data:
                ...
                try:
                    if action not in action_map.all:
                        failed.append(data.update({"error": _("Action '{}' not correct, Action should be in '{}'".format(action, ', '.join(action_map.all)))}))
                        continue
