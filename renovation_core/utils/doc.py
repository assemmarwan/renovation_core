...
def get_doc(doctype, name):
	...
	try:
		...
		update_http_response({"data": "failed", "status": "failed", "exception": ex, "traceback": traceback.format_exc() })
...
def put_doc(doctype, name):
	...
	try:
		...
		if frappe.get_meta(doctype).is_submittable and 1 == 2:
			...
			set_local_name(doc) # mandatory call before saving for new __islocal docs
			doc.save()
