...
def apply_docdefaults(data, doc, doctype, evaluation_time = "Normal"):
	...
	defaults = frappe.get_value('Renovation DocDefaults', filters={'dt': doctype})
	if defaults:
		defaults = frappe.get_all('Renovation DocDefault Items', fields="*", filters={'parent': defaults})
	else:
		defaults = []
	...
	for field in defaults:
		...
		if field.get_value_from == "Static Value":
			set_field_value(doc, field.fieldname, field.link_default_value if field.fieldtype == "Link" else field.default_value, field.override)
	...
	table_docfields = frappe.get_all('DocField', fields=['name', 'fieldname', 'options'], filters={'parent': doctype, 'fieldtype': 'Table'})
	...
	for field in calculated_defaults:
		if field.get_value_from == "Method":
			...
			set_field_value(doc, field.fieldname, f(*params) if field.parameter else f(), field.override)
		elif field.get_value_from == "Evaluate":
			set_field_value(doc, field.fieldname, eval(field.eval_code), field.override)
