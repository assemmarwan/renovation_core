...
io.on('connection', async function (socket) {
  ...
  // [renovation_core] We have many cases when there are no cookies
	...
	const { user, sid } = await getRenovationUserDetails(socket);
	console.log("gotUserDetails", user, sid)
...
function getRenovationUserDetails(socket) {
  ...
  var sid = null;
  if (socket.request.headers.cookie) {
    sid = cookie.parse(socket.request.headers.cookie).sid
  }
  ...
  return new Promise((resolve, rej) => {
    ...
    .end(function (err, res) {
      ...
      if (res.status == 200) {
        result.user = res.body.message.user;
        result.sid = res.body.message.sid;
      }
